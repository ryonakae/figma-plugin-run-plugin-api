...
declare module 'plugin-api-standalone' {
	...
	interface StylePropertyChange extends BaseStyleChange {
	  ...
	  | StyleCreateChange
	  | StyleDeleteChange
	  | StylePropertyChange type NodeChangeProperty =
	  | 'pointCount'
	  | 'name'
	  | 'width'
	  | 'height'
	...
	interface NodeChangeEvent {
	  nodeChanges: NodeChange[]
	} type NodeChange = CreateChange | DeleteChange | PropertyChange type StyleChangeProperty =
	  | 'name'
	  | 'pluginData'
	  | 'type'
	  | 'description'
	  ...
	  | 'paint'
	  | 'effects'
	  | 'layoutGrids' type TextReviewEvent = {
	  text: string
	...
	interface FontName {
	  ...
	  | 'TITLE'
	  | 'SMALL_CAPS'
	  | 'SMALL_CAPS_FORCED' type TextDecoration = 'NONE' | 'UNDERLINE' | 'STRIKETHROUGH' type TextDecorationStyle = 'SOLID' | 'WAVY' | 'DOTTED' type TextDecorationOffset =
	  | {
	      readonly value: number
	      readonly unit: 'PIXELS' | 'PERCENT'
	    }
	...
	} type Effect = DropShadowEffect | InnerShadowEffect | BlurEffect type ConstraintType = 'MIN' | 'CENTER' | 'MAX' | 'STRETCH' | 'SCALE'
	interface Constraints {
	  readonly horizontal: ConstraintType
	  readonly vertical: ConstraintType
	}
	...
	interface ExportSettingsREST {
	  readonly format: 'JSON_REST_V1'
	} type ExportSettings = ExportSettingsImage | ExportSettingsSVG | ExportSettingsPDF type WindingRule = 'NONZERO' | 'EVENODD'
	interface VectorVertex {
	  readonly x: number
	  readonly y: number
	  readonly strokeCap?: StrokeCap
	...
	interface LetterSpacing {
	  ...
	  | {
	      readonly unit: 'AUTO'
	    } type LeadingTrim = 'CAP_HEIGHT' | 'NONE' type HyperlinkTarget = {
	  type: 'URL' | 'NODE'
	  value: string
	  ...
	  | 'SATURATION'
	  | 'COLOR'
	  | 'LUMINOSITY' type MaskType = 'ALPHA' | 'VECTOR' | 'LUMINANCE'
	interface Font {
	  fontName: FontName
	...
	interface StyledTextSegment {
	  ...
	  | 'VARIABLE_ALIAS'
	  | 'COLOR'
	  | 'EXPRESSION' type ExpressionFunction =
	  | 'ADDITION'
	  | 'SUBTRACTION'
	  | 'MULTIPLICATION'
	  | 'DIVISION'
	...
	interface DirectionalTransition {
	  ...
	  readonly easing: Easing
	  readonly duration: number
	} type Transition = SimpleTransition | DirectionalTransition type Trigger =
	  | {
	      readonly type: 'ON_CLICK' | 'ON_HOVER' | 'ON_PRESS' | 'ON_DRAG'
	    }
	  | {
	...
	interface EasingFunctionSpring {
	  ...
	  damping: number
	  initialVelocity: number
	} type OverflowDirection = 'NONE' | 'HORIZONTAL' | 'VERTICAL' | 'BOTH' type OverlayPositionType =
	  | 'CENTER'
	  | 'TOP_LEFT'
	  | 'TOP_CENTER'
	  | 'TOP_RIGHT'
	  ...
	  | {
	      readonly type: 'SOLID_COLOR'
	      readonly color: RGBA
	    } type OverlayBackgroundInteraction = 'NONE' | 'CLOSE_ON_CLICK_OUTSIDE' type PublishStatus = 'UNPUBLISHED' | 'CURRENT' | 'CHANGED'
	interface ConnectorEndpointPosition {
	  position: {
	    x: number
	    y: number
	...
	interface ConnectorEndpointEndpointNodeIdAndMagnet {
	  ...
	  | ConnectorEndpointPosition
	  | ConnectorEndpointEndpointNodeIdAndMagnet
	  | ConnectorEndpointPositionAndEndpointNodeId type ConnectorStrokeCap =
	  | 'NONE'
	  | 'ARROW_EQUILATERAL'
	  | 'ARROW_LINES'
	  | 'TRIANGLE_FILLED'
	...
	interface SceneNodeMixin extends ExplicitVariableModesMixin {
	  ...
	  | 'strokeBottomWeight'
	  | 'strokeLeftWeight'
	  | 'opacity' type VariableBindableTextField =
	  | 'fontFamily'
	  | 'fontSize'
	  | 'fontStyle'
	  | 'fontWeight'
	  ...
	  | 'lineHeight'
	  | 'paragraphSpacing'
	  | 'paragraphIndent' type VariableBindablePaintField = 'color' type VariableBindablePaintStyleField = 'paints' type VariableBindableColorStopField = 'color' type VariableBindableEffectField = 'color' | 'radius' | 'spread' | 'offsetX' | 'offsetY' type VariableBindableEffectStyleField = 'effects' type VariableBindableLayoutGridField = 'sectionSize' | 'count' | 'offset' | 'gutterSize' type VariableBindableGridStyleField = 'layoutGrids' type VariableBindableComponentPropertyField = 'value' type VariableBindableComponentPropertyDefinitionField = 'defaultValue'
	interface StickableMixin {
	  stuckTo: SceneNode | null
	}
	interface ChildrenMixin {
	...
	interface DeprecatedBackgroundMixin {
	  backgrounds: ReadonlyArray<Paint>
	  backgroundStyleId: string
	} type StrokeCap = 'NONE' | 'ROUND' | 'SQUARE' | 'ARROW_LINES' | 'ARROW_EQUILATERAL' type StrokeJoin = 'MITER' | 'BEVEL' | 'ROUND' type HandleMirroring = 'NONE' | 'ANGLE' | 'ANGLE_AND_LENGTH'
	interface AutoLayoutMixin {
	  layoutMode: 'NONE' | 'HORIZONTAL' | 'VERTICAL'
	  layoutWrap: 'NO_WRAP' | 'WRAP'
	  paddingLeft: number
	...
	interface InferredAutoLayoutResult extends AutoLayoutChildrenMixin, AutoLayoutMixin {} type DetachedInfo =
	  | {
	      type: 'local'
	      componentId: string
	    }
	...
	interface Measurement {
	  ...
	  offset: MeasurementOffset
	  freeText: string
	} type MeasurementSide = 'TOP' | 'RIGHT' | 'BOTTOM' | 'LEFT' type MeasurementOffset =
	  | {
	      type: 'INNER'
	      relative: number
	    }
	...
	interface TextNode
	  ...
	  textStyleId: string | PluginAPI['mixed']
	  setTextStyleIdAsync(styleId: string): Promise<void>
	} type ComponentPropertyType = 'BOOLEAN' | 'TEXT' | 'INSTANCE_SWAP' | 'VARIANT' type InstanceSwapPreferredValue = {
	  type: 'COMPONENT' | 'COMPONENT_SET'
	  key: string
	...
	interface VariableAlias {
	  type: 'VARIABLE_ALIAS'
	  id: string
	} type VariableValue = boolean | string | number | RGB | RGBA | VariableAlias type VariableScope =
	  | 'ALL_SCOPES'
	  | 'TEXT_CONTENT'
	  | 'CORNER_RADIUS'
	  | 'WIDTH_HEIGHT'
	  ...
	  | 'LETTER_SPACING'
	  | 'PARAGRAPH_SPACING'
	  | 'PARAGRAPH_INDENT' type CodeSyntaxPlatform = 'WEB' | 'ANDROID' | 'iOS'
	interface Variable extends PluginDataMixin {
	  readonly id: string
	  name: string
	  description: string
	...
	interface SlideTransition {
	...
	} type BaseNode = DocumentNode | PageNode | SceneNode type SceneNode =
	  | SliceNode
	  | FrameNode
	  | GroupNode
	  | ComponentSetNode
	  ...
	  | SlideRowNode
	  | SlideGridNode
	  | InteractiveSlideElementNode type NodeType = BaseNode['type'] type StyleType = 'PAINT' | 'TEXT' | 'EFFECT' | 'GRID' type InheritedStyleField =
	  | 'fillStyleId'
	  | 'strokeStyleId'
	  | 'backgroundStyleId'
	  | 'textStyleId'
